...
class TestExponentialBackoff:
    ...
    def test_exponential_backoff_max_delay_cap(self):
        ...
        assert (
            limiter._calculate_backoff_delay(10) == 10.0
        )  # Would be 1024.0, but capped
    ...
    def test_reset_exponential_backoff_on_success(self):
        ...
        limiter = EnhancedRateLimiter(
            exponential_backoff_enabled=True, base_backoff_delay=0.1
        )
    ...
    def test_exponential_backoff_configuration_validation(self):
        ...
        with pytest.raises(ValueError, match="Base backoff delay must be positive"):
            EnhancedRateLimiter(
                exponential_backoff_enabled=True, base_backoff_delay=0.0
            )

        # Invalid max delay should raise error
        with pytest.raises(ValueError, match="Max backoff delay must be positive"):
            EnhancedRateLimiter(exponential_backoff_enabled=True, max_backoff_delay=0.0)

        # Invalid multiplier should raise error
        with pytest.raises(
            ValueError, match="Backoff multiplier must be greater than 1"
        ):
            EnhancedRateLimiter(
                exponential_backoff_enabled=True, backoff_multiplier=1.0
            )
