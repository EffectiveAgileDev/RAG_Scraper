...
class TestMultiPageProgressNotifier:
    ...
    def test_page_discovery_notification(self):
        ...
        notifier.notify_pages_discovered("Tony's Restaurant", discovered_pages, mock_callback)
        
        assert len(callback_calls) == 1
        assert "Discovered 4 pages for Tony's Restaurant" in callback_calls[0]["message"]
    ...
    def test_page_processing_completion_notification(self):
        ...
        notifier.initialize_restaurant("Tony's Restaurant", ["page1", "page2", "page3"])
        notifier.notify_page_complete("http://restaurant.com/menu", "menu", True, mock_callback)
    ...
    def test_page_processing_failure_notification(self):
        ...
        notifier.initialize_restaurant("Tony's Restaurant", ["page1", "page2", "page3"])
        notifier.notify_page_complete("http://restaurant.com/menu", "menu", False, mock_callback)
        
        call = callback_calls[0]
        assert "Failed to load Menu page - continuing with other pages" in call["message"]
    ...
    def test_restaurant_completion_with_failures(self):
        ...
        call = callback_calls[0]
        assert "Completed Tony's Restaurant (2 of 3 pages successful)" in call["message"]
    ...
    def test_batch_progress_with_multiple_restaurants(self):
        ...
        overall_calls = [call for call in callback_calls if "Restaurant 1 of 3" in call["message"]]
...
class TestMultiPageProgressIntegration:
    ...
    def test_progress_notifications_during_multi_page_scraping(self):
        ...
        with patch.object(scraper, '_fetch_page') as mock_fetch, \
             patch.object(scraper.page_discovery, 'discover_all_pages') as mock_discover:
            ...
            result = scraper.scrape_website("http://restaurant.com", progress_callback=mock_callback)
            
            # Should have discovery, page processing, and completion notifications
            discovery_calls = [call for call in progress_calls if "Discovered" in call["message"]]
            assert len(discovery_calls) == 1
            
            processing_calls = [call for call in progress_calls if "Processing page" in call["message"]]
            assert len(processing_calls) == 3  # One for each page
            
            completion_calls = [call for call in progress_calls if "Completed" in call["message"]]
    ...
    def test_progress_with_page_failures(self):
        ...
        with patch.object(scraper, '_fetch_page') as mock_fetch, \
            ...
            failure_calls = [call for call in progress_calls if "Failed to load" in call["message"]]
            assert len(failure_calls) == 1
            
            # Final completion should indicate partial success
            completion_calls = [call for call in progress_calls if "2 of 3 pages successful" in call["message"]]
            assert len(completion_calls) == 1
    ...
    def test_progress_respects_page_limits(self):
        ...
        with patch.object(scraper.page_discovery, 'discover_all_pages') as mock_discover:
            ...
            with patch.object(scraper, '_fetch_and_process_page') as mock_process:
                ...
                limit_calls = [call for call in progress_calls if "page limit" in call["message"]]
    ...
    def test_detailed_progress_information(self):
        ...
        with patch.object(scraper, '_fetch_and_process_page') as mock_process, \
            ...
            mock_process.return_value = {"page_type": "menu", "data": {"name": "Restaurant"}}
            
            result = scraper.scrape_website("http://restaurant.com", progress_callback=mock_callback)
            
            # Check that progress includes percentage and time estimates
            calls_with_percentage = [call for call in progress_calls if call["percentage"] is not None]
            assert len(calls_with_percentage) > 0
            
            calls_with_time = [call for call in progress_calls if call["time_estimate"] is not None]
