...
class TestMultiPageScraperTraversalStrategies:
    ...
    def test_breadth_first_traversal_order(self):
        ...
        with patch.object(scraper, '_fetch_page') as mock_fetch, \
            ...
            home_index = next(i for i, url in enumerate(visited_order) if url == "http://example.com/")
    ...
    def test_depth_first_traversal_order(self):
        ...
        with patch.object(scraper, '_fetch_page') as mock_fetch, \
            ...
            branch1_index = next((i for i, url in enumerate(visited_order) if url.endswith("/branch1")), -1)
            branch1_deep_index = next((i for i, url in enumerate(visited_order) if "branch1/deep" in url), -1)
            branch2_index = next((i for i, url in enumerate(visited_order) if url.endswith("/branch2")), -1)
    ...
    def test_traversal_respects_max_depth(self):
        ...
        with patch.object(scraper, '_fetch_page') as mock_fetch, \
            ...
            visited_order = scraper._breadth_first_traversal("http://example.com/", max_depth=2)
            
            # Should not go beyond specified depth
            assert all("level3" not in url and "level4" not in url for url in visited_order)
