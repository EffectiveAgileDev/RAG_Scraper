"""Unit tests for enhanced page status display functionality (simplified)."""
...
class TestEnhancedPageStatusDisplaySimple:
    """Simplified test suite for enhanced page status display."""
    ...
    def test_status_indicator_css_classes(self):
        """Test that status indicators have correct CSS classes."""
        success_page = self.mock_results['successful_pages'][0]
        failed_page = self.mock_results['failed_pages'][0]
        timeout_page = self.mock_results['timeout_pages'][0]
        
        success_html = self._format_page_status(success_page)
        failed_html = self._format_page_status(failed_page)
        timeout_html = self._format_page_status(timeout_page)
        
        assert 'status-indicator success' in success_html
        assert 'status-indicator failed' in failed_html
        assert 'status-indicator timeout' in timeout_html
    
    def test_processing_time_display(self):
    ...
    def test_timestamp_formatting(self):
        ...
        timestamp = "2024-01-15T10:30:00Z"
        formatted = self._format_timestamp_for_display(timestamp)
    ...
    def _format_timestamp_for_display(self, timestamp_str):
