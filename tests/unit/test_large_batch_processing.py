...
class TestLargeBatchProcessing:
    ...
    def test_batch_processing_progress_accuracy(self):
        ...
        with patch.object(processor.scraper, 'scrape_url') as mock_scrape:
            mock_scrape.return_value = RestaurantData(name="Test Restaurant", sources=["mock"])
            ...
            percentage_updates = [u['percentage'] for u in progress_updates if u['percentage'] is not None]
            ...
            for i in range(1, len(percentage_updates)):
                assert percentage_updates[i] >= percentage_updates[i-1], \
                    f"Progress went backwards: {percentage_updates[i-1]}% to {percentage_updates[i]}%"
            
            # Verify time estimates appear after first URL
            time_estimates = [u['time_estimate'] for u in progress_updates if u['time_estimate'] is not None]
    ...
    def test_memory_monitoring_accuracy(self):
        ...
        assert initial_stats['current_mb'] < 200, "Initial memory usage should be reasonable"
    ...
    def test_batch_processor_stop_functionality(self):
        ...
        with patch.object(processor.scraper, 'scrape_url') as mock_scrape:
            ...
            if 'result' in result_container:
                ...
                assert result['total_processed'] < 100, "Processing should have been stopped"