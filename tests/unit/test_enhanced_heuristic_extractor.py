...
class TestEnhancedHeuristicExtractor:
    ...
    def test_cross_page_pattern_correlation(self):
        ...
        analyzer.record_pattern_usage(
            "directory", "name", ".restaurant-title", success=True
        )
        analyzer.record_pattern_usage(
            "directory", "name", ".restaurant-title", success=True
        )
        ...
        assert (
            correlations["directory"]["name"][".restaurant-title"]["success_rate"]
            == 1.0
        )
    ...
    def test_extraction_timestamp_tracking(self):
        ...
        with patch(
            "src.scraper.enhanced_heuristic_extractor.datetime"
        ) as mock_datetime:
            mock_dt = Mock()
            ...
            if results:
                assert (
                    results[0].extraction_metadata["timestamp"] == "2024-01-15T10:30:00"
                )
    ...
    def test_pattern_evolution_tracking(self):
        ...
        tracker.record_pattern_change(
            "phone", ".phone", ".contact", reason="page_restructure"
        )
    ...
    def test_contextual_extraction_adjustment(self):
        ...
        menu_context = ExtractionContext(
            entity_id="menu_001", parent_id="rest_001", page_type="menu"
        )
