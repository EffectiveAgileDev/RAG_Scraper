...
class TestScrapingFileGenerationIntegration:
    ...
    def test_scraper_returns_metadata_not_file_paths(
        self, scraping_config, sample_restaurant_data
    ):
        ...
        with patch(
            "src.scraper.restaurant_scraper.MultiStrategyScraper"
            ...
            assert (
                len(text_files) == 0
            ), "Scraper should not create files directly, but it returns success"

    def test_scraper_success_but_no_file_generation(
        self, scraping_config, sample_restaurant_data
    ):
        ...
        with patch(
            ...
            assert (
                len(output_files) == 0
            ), "No output files should exist after scraping alone"

    def test_file_generation_service_works_independently(
        self, temp_output_dir, sample_restaurant_data
    ):
    ...
    def test_missing_integration_between_scraper_and_file_generation(
        self, scraping_config, sample_restaurant_data
    ):
    ...
    def test_flask_scrape_endpoint_missing_file_generation(self, temp_output_dir):
        ...
        with patch(
            ...
            output_files = [
                f for f in os.listdir(temp_output_dir) if f.startswith("WebScrape_")
            ]

            # THIS SHOULD PASS - demonstrating the problem
            assert (
                len(output_files) == 0
            ), "Flask scrape endpoint should create files but doesn't"

    def test_output_files_response_contains_descriptions_not_paths(
        ...
        with patch(
            ...
            assert not output_files[0].startswith(
                "/"
            ), "Response should contain file paths, not descriptions"
            assert not os.path.exists(
                output_files[0]
            ), "Response contains description, not actual file path"
        ...
        with patch(
            ...
            mock_result = ScrapingResult(
                successful_extractions=[
                    RestaurantData(
                        name="Test Restaurant", address="123 Test St", sources=["test"]
                    )
                ],
                failed_urls=[],
                total_processed=1,
