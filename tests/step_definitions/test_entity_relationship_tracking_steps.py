...
def setup_pages(relationship_tracker, sample_pages, table):
    ...
    for row in table:
        ...
        relationship_tracker.create_entity(
            url=page_url, entity_type=page_type, entity_id=entity_id
        )

        # Store in sample_pages for reference
        sample_pages[entity_id] = {"url": page_url, "type": page_type, "id": entity_id}


@when(
    parsers.parse(
        'I track a parent-child relationship from "{parent_id}" to "{child_id}"'
    )
)
...
@when(
    parsers.parse(
        'I track a reference from "{from_id}" to "{to_id}" with type "{ref_type}"'
    )
)
...
@when(
    parsers.parse(
        'I attempt to create a circular reference from "{from_id}" to "{to_id}"'
    )
)
def attempt_circular_reference(relationship_tracker, from_id, to_id):
    """Attempt to create a circular reference."""
    relationship_tracker.track_relationship(
        from_entity=from_id, to_entity=to_id, relationship_type="reference"
    )


@when(
    parsers.parse(
        'I track a relationship from "{from_id}" to "{to_id}" with source page "{source}"'
    )
)
def track_with_source(relationship_tracker, from_id, to_id, source):
    ...
    relationship_tracker.track_relationship(
        ...
        metadata={"source_page": source, "extraction_method": "page_discovery"},
    )
...
@then(
    parsers.parse(
        'the entity "{entity_id}" should have a "{ref_type}" reference to "{target_id}"'
    )
)
def check_reference(relationship_tracker, entity_id, ref_type, target_id):
    ...
    metadata = relationship_tracker.relationships[entity_id]["reference_metadata"][
        target_id
    ]
...
@then(
    parsers.parse(
        'the entity "{entity_id}" should have an incoming reference from "{source_id}"'
    )
)
...
@then(
    parsers.parse(
        'I can query all incoming references to "{entity_id}" and get {expected}'
    )
)
