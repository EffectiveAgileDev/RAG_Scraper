...
@given(
    parsers.parse('the page belongs to entity "{entity_id}" with parent "{parent_id}"')
)
...
def link_menu_entity(entity_tracker, entity_id):
    ...
    entity_tracker.create_entity(
        url="/restaurants/italian-bistro/menu", entity_type="menu", entity_id=entity_id
    )
...
@when(
    parsers.parse(
        'I extract data from detail pages for entities "{entity1}" and "{entity2}"'
    )
)
