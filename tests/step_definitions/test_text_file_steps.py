...
def restaurant_data_minimal():
    ...
    return RestaurantData(name="Simple Cafe", phone="555-1234", sources=["heuristic"])
...
def output_directory_has_permissions(text_file_generator):
    ...
    result = validator.validate_directory_writable(
        text_file_generator.config.output_directory
    )
...
def generate_text_file_for_rag(test_context, text_file_generator):
    ...
    test_context["output_file"] = text_file_generator.generate_file(
...
@then(
    "the output file should contain all restaurants separated by double carriage returns"
)
...
def scraped_multipage_restaurant(test_context):
    ...
    restaurant = RestaurantData(
        ...
        sources=[
            ...
            "microdata",
            "heuristic",
        ],  # Multiple sources indicate multi-page
    )
...
@then(
    "the output should consolidate all multi-page data into a single restaurant entry"
)
...
def mock_current_datetime(test_context):
    ...
    test_context["mocked_datetime"] = patch(
        "src.file_generator.text_file_generator.datetime"
    )
    ...
    test_context["restaurant_data"] = [
        RestaurantData(name="Test Restaurant", sources=["heuristic"])
    ]
...
def restaurant_has_special_characters(test_context, step):
    ...
    assert restaurant.cuisine == expected_values["cuisine"]
    assert any(
        item in str(restaurant.menu_items)
        for item in ["Paella", "tapas", "crème brûlée"]
    )
...
@given(
    'I have previously selected "/home/user/restaurant_data/" as my output directory'
)
...
def select_directory_without_permissions(test_context):
    ...
    test_context[
        "readonly_directory"
    ] = "/root"  # System directory without user write access
