...
def identify_relevant_pages(multi_page_context):
    ...
    page_messages = [
        msg
        for msg in messages
        if any(page in msg for page in ["Menu", "About", "Contact", "Hours"])
    ]
...
def exclude_irrelevant_pages(multi_page_context):
    ...
    irrelevant_messages = [
        msg
        for msg in messages
        if any(page in msg for page in ["Blog", "Careers", "Privacy"])
    ]
...
def should_see_page_progress_updates(multi_page_context):
    ...
    messages = [msg["message"] for msg in multi_page_context["progress_messages"]]
    progress_messages = [
        msg for msg in messages if "Processing page" in msg and "of" in msg
    ]
...
def should_see_page_type_notifications(multi_page_context):
    ...
    page_type_messages = [
        msg
        for msg in messages
        if any(ptype in msg for ptype in ["Menu page", "Contact page", "About page"])
    ]
...
def should_see_completion_message(multi_page_context):
    ...
    messages = [msg["message"] for msg in multi_page_context["progress_messages"]]
    completion_messages = [
        msg for msg in messages if "Completed" in msg and "pages processed" in msg
    ]
...
@given(
    parsers.parse(
        "I provide a restaurant website with data spread across pages:\n{table}"
    )
)
    ...
    with patch.object(scraper, "_fetch_and_process_page") as mock_process:
        ...
        def mock_page_processing(url):
            if "menu" in url:
                ...
                return {
                    "page_type": "about",
                    "data": RestaurantData(
                        name="Restaurant Name", cuisine="Italian", sources=["heuristic"]
                    ),
                }
            else:  # home page
                return {
                    "page_type": "home",
                    "data": RestaurantData(
                        name="Restaurant Name", sources=["heuristic"]
                    ),
                }
        ...
        with patch.object(
            ...
            result = scraper.scrape_website(
                url, progress_callback=mock_progress_callback
            )
            multi_page_context["final_results"][url] = result
...
@given(
    parsers.parse(
        "I provide a restaurant website with conflicting information:\n{table}"
    )
)
...
def should_see_failure_message(multi_page_context):
    ...
    messages = [msg["message"] for msg in multi_page_context["progress_messages"]]
    failure_messages = [
        msg for msg in messages if "Failed to load" in msg and "Menu page" in msg
    ]
...
def final_result_contains_successful_data(multi_page_context):
    ...
    url = (
        multi_page_context["restaurant_urls"][0]
        if multi_page_context["restaurant_urls"]
        else None
    )
...
@given(
    parsers.parse("I provide multiple restaurant URLs for batch processing:\n{table}")
)
        ...
        def mock_batch_processing(config, progress_callback=None):
            ...
            for i, url in enumerate(config.urls):
                if progress_callback:
                    progress_callback(
                        f"Processing restaurant {i+1} of {len(config.urls)}: {url}"
                    )
            ...
            return ScrapingResult(
                successful_extractions=[
                    RestaurantData(name=f"Restaurant {i+1}", sources=["mock"])
...
def final_results_contain_aggregated_data(multi_page_context):
    ...
    assert len(batch_result.successful_extractions) == len(
        multi_page_context["batch_restaurants"]
    )
