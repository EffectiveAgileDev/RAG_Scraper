...
def should_see_progress_indicator(scraper_context):
    ...
    assert len(scraper_context["progress_updates"]) > 0, "No progress updates received"
...
def scraping_completes_successfully(scraper_context):
    ...
    assert (
        scraper_context["error"] is None
    ), f"Scraping failed with error: {scraper_context['error']}"
    assert scraper_context["result"] is not None, "No scraping result returned"
    assert (
        len(scraper_context["result"].successful_extractions) > 0
...
def should_receive_formatted_text_file(scraper_context):
    ...
    assert (
        len(scraper_context["result"].output_files.get("text", [])) > 0
...
def file_contains_restaurant_address(scraper_context):
    ...
    assert (
        "," in address_line or "Street" in address_line or "Ave" in address_line
...
def file_contains_restaurant_phone(scraper_context):
    ...
    assert re.search(phone_pattern, content), "Phone number not found in file"
...
def file_contains_restaurant_hours(scraper_context):
    ...
    assert any(
        keyword in content_lower for keyword in hours_keywords
...
def file_contains_menu_items_with_sections(scraper_context):
    ...
    assert any(
        section in content_upper for section in menu_sections
...
def should_see_error_message(scraper_context):
    ...
    assert (
        scraper_context["error"] is not None
...
def error_specifies_url_validation_failure(scraper_context):
    ...
    assert (
        "url" in scraper_context["error"].lower()
        or "invalid" in scraper_context["error"].lower()
...
def should_see_network_error_message(scraper_context):
    ...
    assert any(
        keyword in error_lower for keyword in network_keywords
...
def error_specifies_connection_failure(scraper_context):
    ...
    assert any(
        keyword in error_lower for keyword in connection_keywords
...
def missing_fields_indicated_as_not_available(scraper_context):
    ...
    assert (
        "Not Available" in content or "N/A" in content
