...
def given_restaurant_data_from_pages(aggregation_context, table):
    ...
    for line in lines[1:]:  # Skip header
        if "|" in line:
            ...
            if len(parts) >= 5:
                page_type, url, name, address, phone = (
                    parts[0],
                    parts[1],
                    parts[2],
                    parts[3],
...
def given_hierarchical_restaurant_data(aggregation_context, table):
    ...
    for line in lines[1:]:  # Skip header
        if "|" in line:
            ...
            if len(parts) >= 5:
                level, url, name, content_type, parent_url = (
                    parts[0],
                    parts[1],
                    parts[2],
                    parts[3],
    ...
    for line in lines[1:]:  # Skip header
        if "|" in line:
            ...
            if len(parts) >= 5:
                ...
                restaurant_data = MockRestaurantData(
                    name=name, url=url, address=address, phone=phone, email=email
                )
                aggregation_context["restaurant_data"].append(restaurant_data)
...
def given_multiple_restaurants_from_directory(aggregation_context, table):
    ...
    for line in lines[1:]:  # Skip header
        if "|" in line:
            ...
            if len(parts) >= 4:
                name, main_url, menu_url, review_url = (
                    parts[0],
                    parts[1],
                    parts[2],
                    parts[3],
                ...
                main_data = MockRestaurantData(
                    name=name, url=main_url, page_type="main"
                )
                aggregation_context["restaurant_data"].append(main_data)

                # Add menu page
                menu_data = MockRestaurantData(
                    name=name, url=menu_url, page_type="menu", parent_url=main_url
                )
                aggregation_context["restaurant_data"].append(menu_data)

                # Add review page
                review_data = MockRestaurantData(
                    name=name, url=review_url, page_type="reviews", parent_url=main_url
                )
                aggregation_context["restaurant_data"].append(review_data)


@given(
    parsers.parse("I have aggregated restaurant data with multiple levels:\\n{table}")
)
def given_aggregated_multilevel_data(aggregation_context, table):
    ...
    for line in lines[1:]:  # Skip header
        if "|" in line:
            ...
            if len(parts) >= 5:
                level, entity_type, name, parent, children_count = (
                    parts[0],
                    parts[1],
                    parts[2],
                    parts[3],
...
@given(
    parsers.parse(
        "I have overlapping restaurant data from different sources:\\n{table}"
    )
)
...
def when_aggregate_restaurant_data(aggregation_context):
    ...
    for data in restaurant_data:
        entity = RestaurantEntity(
            ...
            data={"address": data.address, "phone": data.phone, "email": data.email},
        )
...
@then(
    parsers.parse(
        "the restaurant profile should contain all data from the {page_count:d} pages"
    )
)
...
def then_should_maintain_parent_child_relationships(aggregation_context):
    ...
    for rel in relationships:
        ...
        ), "Should be parent-child relationship"
...
def then_directory_should_reference_main_page(aggregation_context):
    ...
    dir_relationships = [
        r for r in relationships if r.parent_id == directory_entity.entity_id
    ]
...
def then_main_page_should_reference_subpages(aggregation_context):
    ...
    main_relationships = [
        r for r in relationships if r.parent_id == main_entity.entity_id
    ]
    assert (
        len(main_relationships) >= 2
    ), "Main page should reference menu and hours pages"
...
def then_pages_should_know_hierarchy_level(aggregation_context):
    ...
    for entity in entities:
        assert (
            "level" in entity.data
        ), f"Entity {entity.entity_id} should have level data"
...
def then_should_have_restaurant_profiles(aggregation_context, count):
    ...
    assert (
        len(result) == count
    ), f"Expected {count} profiles after deduplication, got {len(result)}"


@then(
    parsers.parse(
        'the restaurant should have the most complete address "{expected_address}"'
    )
)
def then_should_have_complete_address(aggregation_context, expected_address):
    ...
    entity = result[0]
    assert (
        entity.data.get("address") == expected_address
    ), f"Expected address '{expected_address}'"


@then(
    parsers.parse(
        'the restaurant should have normalized phone number "{expected_phone}"'
    )
)
def then_should_have_normalized_phone(aggregation_context, expected_phone):
    ...
    entity = result[0]
    assert (
        entity.data.get("phone") == expected_phone
    ), f"Expected phone '{expected_phone}'"
...
def then_should_have_restaurant_entities(aggregation_context, count):
    ...
    ), f"Expected {count} restaurant entities, got {len(restaurants)}"
...
def then_restaurants_should_be_linked_to_menu(aggregation_context):
    ...
    for menu_entity in menu_entities:
        ...
        assert (
            len(menu_rels) > 0
        ), f"Menu {menu_entity.entity_id} should have parent relationship"
...
def then_restaurants_should_be_linked_to_reviews(aggregation_context):
    ...
    for review_entity in review_entities:
        review_rels = [
            r for r in relationships if r.child_id == review_entity.entity_id
        ]
        assert (
            len(review_rels) > 0
        ), f"Review {review_entity.entity_id} should have parent relationship"
...
@then(
    parsers.parse("the Restaurant Guide should be the root with {children:d} children")
)
def then_guide_should_be_root_with_children(aggregation_context, children):
    ...
    assert (
        root_entity.data.get("children_count") == children
    ), f"Root should have {children} children"
...
def then_marios_should_have_children(aggregation_context, children):
    ...
    assert (
        marios_entity.data.get("children_count") == children
    ), f"Mario's should have {children} children"
...
def then_entities_should_maintain_references(aggregation_context):
    ...
    for entity in structure:
        if entity.data.get("level", 0) > 0:
            assert entity.data.get(
                "parent"
            ), "Non-root entities should have parent reference"
...
def then_merged_should_have_name(aggregation_context, expected_name):
    ...
    assert (
        merged.name == expected_name
    ), f"Expected name '{expected_name}', got '{merged.name}'"
...
def then_merged_should_have_phone(aggregation_context, expected_phone):
    ...
    merged = aggregation_context["merged_data"]
    assert (
        merged.data.get("phone") == expected_phone
    ), f"Expected phone '{expected_phone}'"


@then(
    parsers.parse(
        'the merged restaurant should have the most detailed hours "{expected_hours}"'
    )
)
def then_merged_should_have_detailed_hours(aggregation_context, expected_hours):
    ...
    merged = aggregation_context["merged_data"]
    assert (
        merged.data.get("hours") == expected_hours
...
def then_merged_should_have_cuisine(aggregation_context, expected_cuisine):
    ...
    merged = aggregation_context["merged_data"]
    assert (
        merged.data.get("cuisine") == expected_cuisine
    ), f"Expected cuisine '{expected_cuisine}'"


@then(
    parsers.parse(
        'the merged restaurant should have the highest rating "{expected_rating:f}"'
    )
)
def then_merged_should_have_highest_rating(aggregation_context, expected_rating):
    ...
    merged = aggregation_context["merged_data"]
    assert (
        merged.data.get("rating") == expected_rating
    ), f"Expected rating {expected_rating}"
...
def then_merge_should_preserve_source_info(aggregation_context):
    ...
    assert "sources" in merged.data or hasattr(
        merged, "sources"
