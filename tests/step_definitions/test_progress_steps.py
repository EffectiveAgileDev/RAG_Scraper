...
@given(
    parsers.parse("I have multiple restaurant URLs for time estimation:\n{urls_table}")
)
    ...
    with patch.object(scraper.multi_scraper, "scrape_url") as mock_scrape:
        ...
        try:
            result = scraper.scrape_restaurants(
                config, progress_callback=mock_progress_callback
            )
...
def submit_urls_for_batch_scraping(progress_context, mock_progress_callback):
    ...
    config = ScrapingConfig(urls=progress_context["urls"])
    config.force_batch_processing = (
        True  # Force batch processing for better progress tracking
    )
    scraper = RestaurantScraper()
...
def should_see_initial_progress_message(progress_context):
    ...
    assert (
        len(progress_context["progress_messages"]) > 0
    ...
    assert any(
        msg in first_message for msg in initial_messages
...
def should_see_specific_message(progress_context, expected_message):
    ...
    assert any(
        expected_message.lower() in msg.lower() for msg in messages
...
def should_see_progress_percentage_updates(progress_context):
    ...
    assert (
        len(progress_context["progress_percentages"]) > 0
...
def should_see_completion_message(progress_context):
    ...
    assert any(
        any(keyword in msg.lower() for keyword in completion_keywords)
        for msg in messages
...
def progress_should_go_from_zero_to_hundred(progress_context):
    ...
    assert (
        percentages[-1] >= 90
    ), f"Progress should end near 100%, got {percentages[-1]}%"
...
def should_see_overall_batch_progress(progress_context):
    ...
    assert any(
        any(keyword in msg.lower() for keyword in batch_keywords) for msg in messages
...
def should_see_current_url_being_processed(progress_context):
    ...
    assert any(
        any(url in msg for url in urls) for msg in messages
...
def should_see_pattern_message(progress_context, expected_pattern):
    ...
    if "of" in expected_pattern and "Processing" in expected_pattern:
        pattern_found = any(
            "processing" in msg.lower() and "of" in msg.lower() for msg in messages
        )
        assert pattern_found, f"Pattern '{expected_pattern}' not found in: {messages}"
    else:
        assert any(
            expected_pattern.lower() in msg.lower() for msg in messages
...
def should_see_final_completion_with_statistics(progress_context):
    ...
    assert any(
        any(keyword in msg.lower() for keyword in stats_keywords)
...
def overall_progress_continues_to_next_url(progress_context):
    ...
    if len(progress_context["urls"]) > 1:
        assert any(
            any(keyword in msg.lower() for keyword in continue_keywords)
            for msg in messages
...
def should_see_message_initially(progress_context, initial_message):
    ...
    assert any(
        initial_message.lower() in msg.lower() for msg in messages
...
def should_see_time_estimate_after_first_url(progress_context):
    ...
    assert all(
        est > 0 for est in time_estimates
...
def time_estimate_should_update(progress_context):
    ...
    if len(time_estimates) > 1:
        ...
        assert not all(
            est == time_estimates[0] for est in time_estimates
...
def should_see_page_count_message(progress_context, page_count):
    ...
    assert any(
        f"found {page_count}" in msg.lower() or f"{page_count} pages" in msg.lower()
        for msg in messages
...
@then(
    parsers.parse(
        'I should see "Processing page {page_num:d} of {total_pages:d}: {page_type}"'
    )
)
def should_see_page_processing_message(
    progress_context, page_num, total_pages, page_type
):
    ...
    assert any(
        page_pattern in msg.lower() and type_pattern in msg.lower() for msg in messages
...
def should_see_current_operation_status(progress_context):
    ...
    assert any(
        any(keyword in msg.lower() for keyword in status_keywords) for msg in messages
