...
class PromptTemplate:
    ...
    def __init__(self, industry: str, categories: List[str], custom_instructions: str = ""):
...
class LLMResponse:
    ...
    def __init__(self, response_data: Dict[str, Any], validate: bool = True):
        ...
        self.source_attribution = response_data.get("source_attribution", "LLM extraction")
        
        if validate:
            self.extractions = self._validate_extractions(response_data.get("extractions", []))
        else:
            self.extractions = response_data.get("extractions", [])
    
    def _validate_extractions(self, extractions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for extraction in extractions:
            ...
            if "category" not in extraction or "extracted_data" not in extraction:
                continue
...
class LLMExtractor:
    ...
    def _generate_prompt(self, content: str, industry: str, industry_config: Dict[str, Any]) -> str:
    ...
    def extract(self, content: str, industry: str, industry_config: Dict[str, Any] = None,
        ...
        try:
            ...
            # Track token usage
            if self.track_stats and hasattr(response, 'usage') and response.usage:
                ...
                if isinstance(prompt_tokens, int):
                    self._stats["token_usage"]["prompt_tokens"] += prompt_tokens
                if isinstance(completion_tokens, int):
                    self._stats["token_usage"]["completion_tokens"] += completion_tokens
            ...
            if self.track_stats:
                ...
                for extraction in filtered_extractions:
                    self._stats["confidence_scores"].append(extraction.get("confidence", 0.0))
    ...
    def extract_batch(self, contents: List[str], industry: str, 
                     industry_config: Dict[str, Any] = None) -> List[Dict[str, Any]]:
