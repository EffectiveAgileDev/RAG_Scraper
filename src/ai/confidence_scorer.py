...
class ConfidenceScorer:
    ...
    def calculate_confidence(self, extraction_data: Dict[str, Any]) -> float:
        ...
        if "source_reliability" in weights:
            factors["source_reliability"] = self._calculate_source_reliability(extraction_data)
        
        if "extraction_method" in weights:
            factors["extraction_method"] = self._calculate_extraction_method_score(extraction_data)
        
        if "content_quality" in weights:
            factors["content_quality"] = self._calculate_content_quality(extraction_data)
        
        if "industry_relevance" in weights:
            factors["industry_relevance"] = self._calculate_industry_relevance(extraction_data)
        
        if "llm_confidence" in weights:
            factors["llm_confidence"] = self._calculate_llm_confidence(extraction_data)
        ...
        for factor_name, calculator in self.custom_factors.items():
            if factor_name in weights:
                try:
                    ...
                    logger.warning(f"Custom factor {factor_name} calculation failed: {e}")
    ...
    def _calculate_source_reliability(self, data: Dict[str, Any]) -> float:
        ...
        if source_url:
            ...
            if any(suspicious in domain for suspicious in ["temp", "test", "staging"]):
                score -= 0.2
        ...
        if last_updated:
            try:
                ...
                days_old = (datetime.now().replace(tzinfo=None) - update_date.replace(tzinfo=None)).days
                if days_old < 30:
                    score += 0.1
                elif days_old > 365:
                    score -= 0.1
    ...
    def _calculate_extraction_method_score(self, data: Dict[str, Any]) -> float:
    ...
    def _calculate_industry_relevance(self, data: Dict[str, Any]) -> float:
        ...
        industry_keywords = {
            "restaurant": ["menu", "dining", "food", "restaurant", "cuisine", "chef", "table", "reservation"],
            "medical": ["doctor", "medical", "health", "patient", "treatment", "clinic", "hospital", "appointment"],
            "real estate": ["property", "house", "home", "real estate", "agent", "listing", "price", "bedroom"],
            "dental": ["dental", "dentist", "teeth", "oral", "hygiene", "cleaning", "cavity", "braces"]
        }
        ...
        score = keyword_density * 0.8 + 0.2  # 20% base + up to 80% from keywords
    ...
    def _apply_scoring_method(self, factors: Dict[str, float], 
    ...
    def add_custom_factor(self, name: str, calculator: Callable, weight: float):
    ...
    def filter_by_confidence(self, extraction_data: Union[List[Dict[str, Any]], Dict[str, Any]], 
    ...
    def explain_confidence(self, extraction_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "source_reliability" in weights:
            explanation["source_reliability"] = self._calculate_source_reliability(extraction_data)
        
        if "extraction_method" in weights:
            explanation["extraction_method"] = self._calculate_extraction_method_score(extraction_data)
        
        if "content_quality" in weights:
            explanation["content_quality"] = self._calculate_content_quality(extraction_data)
        
        if "industry_relevance" in weights:
            explanation["industry_relevance"] = self._calculate_industry_relevance(extraction_data)
        
        if "llm_confidence" in weights:
            explanation["llm_confidence"] = self._calculate_llm_confidence(extraction_data)
        
        # Overall confidence
        explanation["overall_confidence"] = self.calculate_confidence(extraction_data)
        explanation["weights_used"] = weights.copy()
        explanation["scoring_method"] = self.scoring_method.value
        
        return explanation
    
    def score_batch(self, extraction_batch: List[Dict[str, Any]]) -> List[float]:
    ...
    def adjust_weights_from_performance(self, performance_data: Dict[str, Dict[str, Any]]):
        ...
        for factor, metrics in performance_data.items():
            if factor in self.weights:
                ...
                confidence_in_metric = min(1.0, samples / 100)  # More samples = more confidence
                adjustment = (accuracy - 0.5) * confidence_in_metric * 0.1  # Max 10% adjustment
    ...
    def analyze_confidence_trends(self, time_series_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        confidences = [data.get("confidence", 0.0) for data in time_series_data]
