...
class FileCreationResult:
    ...
    def __init__(
        self, full_path: str, can_create: bool, error_message: Optional[str] = None
    ):
...
class FilePermissionValidator:
    ...
    def _test_directory_writability(
        self, directory_path: str
    ) -> DirectoryValidationResult:
    ...
    def validate_file_creation(
        self, directory: str, filename: str, create_dirs: bool = False
    ) -> FileCreationResult:
        ...
        if not self.is_filename_safe(filename):
            ...
            return FileCreationResult(
                os.path.join(directory, filename), False, error_msg
            )
        ...
        if file_dir != directory:
            ...
            if not os.path.exists(file_dir):
                try:
                    ...
                    return FileCreationResult(
                        full_path, False, f"Cannot create subdirectories: {str(e)}"
                    )
        ...
        if os.path.exists(full_path):
            if os.path.isdir(full_path):
                return FileCreationResult(
                    full_path, False, "Path exists and is a directory, not a file"
                )
    ...
    def validate_disk_space(
        self, directory: str, required_bytes: int
    ) -> DiskSpaceResult:
        ...
        if required_bytes < 0:
            return DiskSpaceResult(
                ...
                required_bytes,
                False,
                ValidationMessages.invalid_path_input(
        ...
        try:
            ...
            if not has_space:
                error_msg = ValidationMessages.insufficient_space(
                    required_bytes, available_bytes
                )
                return DiskSpaceResult(
                    available_bytes, required_bytes, False, error_msg
                )
    ...
    def create_directory_if_needed(
        ...
        try:
            if os.path.exists(directory_path):
                if os.path.isdir(directory_path):
                    return DirectoryCreationResult(
                        directory_path, False, "Directory already exists", None
                    )
                else:
                    return DirectoryCreationResult(
                        directory_path,
            ...
            return DirectoryCreationResult(
                directory_path, True, "Directory created successfully", None
            )
