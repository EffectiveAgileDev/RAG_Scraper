...
class URLValidator:
    ...
    def validate_url(self, url: str) -> ValidationResult:
        ...
        if not parsed.scheme:
            return ValidationResult(
                url, False, "URL must include a scheme (http:// or https://)"
            )
        ...
        if any(char in url for char in self.DANGEROUS_CHARS):
            return ValidationResult(
                url, False, "URL contains potentially dangerous characters"
            )
    ...
    def sanitize_url(self, url: str) -> SanitizationResult:
        ...
        return SanitizationResult(
            original_url, sanitized_url, changes_made, change_description
        )
