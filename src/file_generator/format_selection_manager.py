...
class FormatSelectionManager:
    ...
    def __init__(self, initial_mode: Union[str, SelectionMode] = SelectionMode.SINGLE):
        ...
        logger.debug(f"FormatSelectionManager initialized with mode: {self._selection_mode.value}")
    ...
    def set_selection_mode(self, mode: Union[str, SelectionMode]) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._selection_mode == SelectionMode.MULTIPLE and new_mode == SelectionMode.SINGLE:
                if len(self._selected_formats) > 1:
                    logger.info("Switching to single mode, keeping only first selected format")
    ...
    def select_format(self, format_name: str, field_selection: Optional[Dict[str, bool]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self._is_valid_format(format_name):
                raise FormatSelectionError(f"Unsupported format: {format_name}. "
            ...
            logger.debug(f"Format selected: {format_name} with mode: {self._selection_mode.value}")
            return {"success": True}
    ...
    def set_format_priority(self, priorities: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            invalid_formats = [fmt for fmt in priorities if not self._is_valid_format(fmt)]
            if invalid_formats:
                raise FormatSelectionError(f"Invalid formats in priority list: {', '.join(invalid_formats)}")
    ...
    def get_export_instructions(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "configurations": {fmt: config.copy() for fmt, config in self._format_configurations.items()},
    ...
    def save_configuration(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "selection_mode": self._selection_mode.value,
            "format_configurations": {fmt: config.copy() for fmt, config in self._format_configurations.items()},
        ...
        try:
            ...
            for fmt in selected:
                if not self._is_valid_format(fmt):
                    raise FormatSelectionError(f"Invalid format in configuration: {fmt}")
            self._selected_formats = selected.copy()
            
            # Load configurations
            self._format_configurations = config_data.get("format_configurations", {}).copy()
    ...
    def _validate_and_convert_mode(self, mode: Union[str, SelectionMode]) -> SelectionMode:
        ...
        if isinstance(mode, str):
            try:
                ...
                raise FormatSelectionError(f"Invalid selection mode: {mode}. Use 'single' or 'multiple'")
        
        raise FormatSelectionError(f"Selection mode must be string or SelectionMode enum, got: {type(mode)}")
    
    def _is_valid_format(self, format_name: str) -> bool:
        """Check if format name is valid."""
        return format_name in self._supported_formats
    
    def _validate_format_configuration(self, format_name: str, field_selection: Optional[Dict[str, bool]]) -> None:
        """Validate format-specific configuration."""
    ...
    def _validate_json_field_selection(self, field_selection: Dict[str, bool]) -> None:
        ...
        for field_name, value in field_selection.items():
            if field_name not in self.VALID_JSON_FIELDS:
                raise FormatSelectionError(f"Invalid JSON field: {field_name}. "
                                         f"Valid fields: {', '.join(self.VALID_JSON_FIELDS)}")
            
            if not isinstance(value, bool):
                raise FormatSelectionError(f"JSON field value must be boolean: {field_name}")
    ...
    def _store_format_configuration(self, format_name: str, field_selection: Optional[Dict[str, bool]]) -> None:
