...
class ExtractionMetricsTracker:
    ...
    def record_extraction(self, entity_id: str, method: str, success: bool, confidence: str = None):
        ...
        if confidence:
            ...
            self._metrics['confidence_scores'].append(confidence_values.get(confidence, 0.5))
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if metrics['total_pages'] > 0:
            metrics['extraction_success_rate'] = metrics['successful_extractions'] / metrics['total_pages']
        else:
...
class JSONLDExtractor:
    ...
    def extract_from_html(self, html_content: str) -> List[EnhancedJSONLDExtractionResult]:
        ...
        if self.extraction_context.page_type == "detail" and results:
            ...
            restaurant_results = [r for r in results if "restaurant" in r.source.lower()]
            if restaurant_results:
                results = restaurant_results[:1]  # Take only the most relevant
                
        return results
        
    def extract_restaurant_data(self, json_data: Any) -> Optional[EnhancedJSONLDExtractionResult]:
    ...
    def _calculate_confidence_with_context(self, json_data: Dict[str, Any]) -> str:
        ...
        if (self.extraction_context.sibling_patterns and 
            json_data.get('@type') == self.extraction_context.sibling_patterns.get('@type')):
