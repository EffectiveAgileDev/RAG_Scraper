...
class EntityRelationshipTracker:
    ...
    def create_entity(
        self, url: str, entity_type: str, entity_id: Optional[str] = None
    ) -> str:
    ...
    def track_relationship(
        ...
        if from_entity not in self.entities or to_entity not in self.entities:
            raise ValueError(
                f"Entity not found: {from_entity if from_entity not in self.entities else to_entity}"
            )
    ...
    def _track_parent_child(
        self, parent: str, child: str, metadata: Optional[Dict] = None
    ):
    ...
    def _track_sibling(
        self, entity1: str, entity2: str, metadata: Optional[Dict] = None
    ):
    ...
    def _track_reference(
        self, from_entity: str, to_entity: str, metadata: Optional[Dict] = None
    ) -> bool:
        """Track reference relationship with circular reference detection."""
        # Check for circular reference
        if (
            to_entity in self.relationships
            and from_entity in self.relationships[to_entity]["references"]
        ):
            # Mark as bidirectional instead of creating circular reference
            self.relationships[from_entity]["reference_metadata"][to_entity] = (
                metadata or {}
            )
            self.relationships[from_entity]["reference_metadata"][to_entity][
                "bidirectional"
            ] = True
            self.relationships[to_entity]["reference_metadata"][from_entity][
                "bidirectional"
            ] = True
