...
class MultiPageProgressNotifier:
    ...
    def notify_page_complete(
        ...
        if success:
            ...
            page_display = self.get_page_type_display(page_type)
            failure_message = f"Failed to load {page_display} - continuing with other pages"
        ...
        message = f"Completed page {self.state.current_page} of {self.state.total_pages}"
    ...
    def get_current_progress_summary(self) -> dict:
        ...
        return {
            ...
            'restaurant_progress': f"{self.state.current_restaurant_index + 1}/{self.state.total_restaurants}" if self.state.total_restaurants > 1 else "1/1",
