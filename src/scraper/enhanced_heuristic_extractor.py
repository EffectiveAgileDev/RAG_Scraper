...
class PatternLearner:
    ...
    def get_best_patterns(self, field: str) -> List[Dict[str, Any]]:
        ...
        return sorted(
            patterns, key=lambda x: (x["success_rate"], x["attempts"]), reverse=True
        )
...
class CrossPagePatternAnalyzer:
    ...
    def record_pattern_usage(
        self, page_type: str, field: str, selector: str, success: bool
    ):
        ...
        if selector not in self._page_patterns[page_type][field]:
            self._page_patterns[page_type][field][selector] = {
                "successes": 0,
                "attempts": 0,
            }
...
class AdaptivePatternSelector:
    ...
    def add_pattern_result(
        self, field: str, selector: str, page_type: str, success: bool
    ):
    ...
    def get_best_patterns_for_page_type(self, page_type: str) -> Dict[str, Dict]:
        ...
        for key, stats in self._pattern_results.items():
            if key.startswith(f"{page_type}:"):
                ...
                if (
                    field not in best_patterns
                    or confidence > best_patterns[field]["confidence"]
                ):
                    best_patterns[field] = {
                        "selector": selector,
...
class PatternEvolutionTracker:
    ...
    def record_pattern_change(
        self, field: str, old_pattern: str, new_pattern: str, reason: str
    ):
...
class SiblingPatternSharer:
    ...
    def share_pattern(
        self, entity_id: str, field: str, selector: str, success_rate: float
    ):
    ...
    def get_patterns_for_sibling(
        self, target_entity: str, source_entity: str
    ) -> Dict[str, Dict]:
...
class ExtractionFailureRecovery:
    ...
    def get_fallback_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "phone": ["div[data-phone]", '*[class*="contact"]', '*[class*="phone"]'],
            "address": ['*[class*="address"]', '*[class*="location"]'],
        }

    def attempt_fallback_extraction(
        self, html: str, fallback_patterns: Dict[str, List[str]]
    ) -> Dict[str, Any]:
...
class PageStructureAnalyzer:
    ...
    def analyze_structure(self, html: str) -> Dict[str, Any]:
        ...
        structure = {"header_elements": [], "content_hierarchy": {}, "max_depth": 0}
...
class PatternOptimizer:
    ...
    def add_pattern_stats(
        self, field: str, selector: str, success_rate: float, usage_count: int
    ):
        ...
        self._pattern_stats[field].append(
            {
                ...
                "score": success_rate
                * (1 + min(usage_count / 100, 1)),  # Boost for high usage
            }
        )
    ...
    def get_optimized_patterns(self, field: str) -> List[Dict]:
        ...
        return sorted(
            self._pattern_stats[field], key=lambda x: x["score"], reverse=True
        )
...
class HeuristicExtractor:
    ...
    def extract_from_html(
    ...
    ) -> List[EnhancedHeuristicExtractionResult]:
        ...
        if hasattr(self, "_pattern_confidence_boost"):
            ...
            ] = self._pattern_confidence_boost
    ...
    def _is_restaurant_page(self, html_content: str) -> bool:
        ...
        keyword_count = sum(
            1 for keyword in self.RESTAURANT_KEYWORDS if keyword in content_lower
        )
    ...
    def _extract_cuisine(self, soup: BeautifulSoup) -> str:
        ...
        found_cuisines = [
            cuisine for cuisine in self.CUISINE_KEYWORDS if cuisine in text
        ]
    ...
    def _calculate_confidence_with_patterns(
        ...
        # Pattern boost
        if (
            hasattr(self.extraction_context, "pattern_learner")
            and self.extraction_context.pattern_learner
        ):
